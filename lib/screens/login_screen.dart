...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void _switchMode(bool createAccount) {
    ...
    setState(() {
      ...
      if (createAccount) {
        // Allow creating another account even if an account
        // already exists on this phone.
        _usernameController.clear();
        _emailController.clear();
      } else if (_savedEmail != null && _savedEmail!.isNotEmpty) {
        // Keep the existing convenience of showing the last
        // used email when switching to Sign In.
      ...
      if (response.statusCode == 200 && body['error'] == null) {
        ...
        if (_isCreateAccount) {
          ...
          setState(() {
            ...
            _loading = false;
            _errorMsg =
                'Account created successfully. Now sign in with your email.';
          });
        } else {
          ...
          final username =
              account is Map ? account['username']?.toString() : null;
          final normalizedEmail =
              account is Map && account['email'] != null
                ...
                child: SingleChildScrollView(
                  ...
                  child: ConstrainedBox(
                    constraints: BoxConstraints(
                      minHeight:
                          constraints.maxHeight - (isDesktop ? 68 : 44),
                    ),
                    child: Column(
                      ...
                      children: [
                        ...
                        if (isDesktop)
                                ...
                                child: Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      SizedBox(height: 18),
                                      Text(
                                        'Use location intelligence, ML prediction and community reports to make safer decisions around wildlife-sensitive areas.',
                                        style: TextStyle(
                          ...
                          const Text(
                            'Predict wildlife risk before it becomes a problem.',
                            style: TextStyle(
                              color: Colors.white,
                              fontSize: 28,
                              height: 1.15,
            ...
            child: Container(
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  ...
                  colors: [
                    forestDark.withOpacity(.94),
                    forest.withOpacity(.78),
                    const Color(0xFF1D633B).withOpacity(.86),
                  ],
                ),
              ),
            ),
        ...
        children: [
          ...
          Positioned.fill(
            child: ClipRRect(
              borderRadius: BorderRadius.circular(24),
              child: Container(
                color: forestDark.withOpacity(.68),
              ),
            ),
          ),
          Padding(
  ...
  Widget _modeSwitch() {
    return Container(
      ...
      child: Row(
        children: [
          Expanded(
            child: _modeButton('Sign in', false),
          ),
          Expanded(
            child: _modeButton('Create account', true),
          ),
        ],
      ),
    );
  ...
  Widget _errorBox() {
    final success =
        _errorMsg.toLowerCase().contains('account created');
    return Container(
      width: double.infinity,
...
class _ForestPatternPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final softPaint = Paint()
      ..color = const Color(0xFFB8D98C).withValues(alpha: 0.035);
